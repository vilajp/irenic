...
class IngresosPage(BoxLayout):
	...
	def agrego_art_ing(self, instance):
		...
		box_confirmacion = BoxLayout(orientation = "vertical")
		grid_confirmacion = GridLayout(cols = 1,
							row_force_default=True, 
							row_default_height=40,
							)
		scroll_confirmacion = ScrollView(size=(Window.width, Window.height),)
		scroll_confirmacion.add_widget(grid_confirmacion)
		box_confirmacion.add_widget(scroll_confirmacion)
		for cada_item in lista_pop:


			itemLabel = Label(text = f"{cada_item}",
							size_hint = (1, .1),
							) 
			
			grid_confirmacion.add_widget(itemLabel)
		...
		box_confirmacion.add_widget(closeButton)        

		self.popup = Popup(title ='UD. INGRESO', 
					  content = box_confirmacion, 
